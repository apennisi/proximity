...
Homography::calcProjection(const cv::Point2f &point, const cv::Mat &H, double &x, double &y) 
{
    ...
    cv::Mat point2D = (cv::Mat_<float>(3, 1) << point.x, point.y, 1);
    cv::Mat point3D = H*point2D;
    x = point3D.at<float>(0) / point3D.at<float>(2);
    y = point3D.at<float>(1) / point3D.at<float>(2);
}
